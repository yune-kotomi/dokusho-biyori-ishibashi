...
class UserTest < ActiveSupport::TestCase
  ...
  test "tagsは{タグ文字列=>総数}の形式で返される" do
    @public_user.tag_table.each do |key, value|
  ...
  test "tagsはjsonとして保存する" do
    source = {'a' => 3, 'b' => 2}
    @public_user.update_attributes(:tags => source.to_json)
    @public_user.reload
  ...
  test "タグの追加・削除がupdate_tagsで行える" do
    ...
    assert_equal ({'tag1' => 1, 'tag2' => 1}), @public_user.tag_table

    @public_user.update_tags(['tag1', 'tag3'], ['tag2'])
    assert_equal ({'tag1' => 2, 'tag3' => 1}), @public_user.tag_table

    @public_user.update_tags(['tag2'], ['tag1'])
    assert_equal ({'tag1' => 1, 'tag2' => 1, 'tag3' => 1}), @public_user.tag_table
  end
