# 商品リストからのタグ付け
Document.ready? do
  if Element.find('.product.modal').size > 0
    def split_tags(src)
    ...
    Element.find('.product.modal .button.tags-update').on('click') do |event|
      user_id = Element.find('body')['data-user-id']
      ...
      HTTP.post(form['action'], :payload => payload) do |response|
        if response.ok?
          # タグ一覧表示を更新
          tag_list = tags.map{|tag| {'value' => tag, 'link' => "/shelfs/show/#{user_id}?keyword=%5B#{u(tag)}%5D"} }
